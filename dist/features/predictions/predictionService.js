...
class PredictionService {
    async runPrediction(customerID) {
        ...
        const predictionLog = await predictionRepository.savePrediction({
            ...
            customerRiskFactors: riskFactors,
            logTopFactors: flaskPrediction.top_factors,
            modelVersion,
    ...
    async getDistribution() {
        ...
        for (const customer of customers) {
            const index = Math.min(Math.floor(customer.churnProbability / 0.2), buckets.length - 1);
            buckets[index].count += 1;
        }
        return buckets;
    }
    async callPredictionService(modelFeatures) {
        ...
        if (!response.ok) {
            throw new AppError(body?.message ?? 'Failed to process prediction', 502);
        }
        return FlaskPredictionSchema.parse(body);
    }
    normalizeTopFactors(factors) {
        const maxAbsShap = Math.max(...factors.map((factor) => Math.abs(factor.shap_value)), 1);
        return factors.map((factor) => ({
            feature: factor.feature,
            shapValue: factor.shap_value,
            impact: Math.round((Math.abs(factor.shap_value) / maxAbsShap) * 1000) /
                1000,
            direction: factor.direction === 'increases_risk' ? 'increases' : 'decreases',
        }));
    }
}
